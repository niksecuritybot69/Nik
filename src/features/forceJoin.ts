...
// Returns true only if the user has joined every configured required
// channel (1st, and 2nd if set).
async function hasJoinedAllRequired(telegram: any, config: any, userId: number): Promise<boolean> {
  if (config.forceJoinChannel) {
    const joined1 = await isChannelMember(telegram, config.forceJoinChannel, userId);
    if (!joined1) return false;
  }
  if (config.forceJoinChannel2) {
    const joined2 = await isChannelMember(telegram, config.forceJoinChannel2, userId);
    if (!joined2) return false;
  }
  return true;
}
...
async function sendVerificationGate(
  ...
  if (!config.forceJoinChannel && !config.forceJoinChannel2) return;
  ...
  const channelWord = config.forceJoinChannel && config.forceJoinChannel2 ? "channels" : "channel";
  const text =
    `<b><i>${chatTitle} Security Bot</i></b>\n\n` +
    `${mention} [${userId}] to be accepted in the group, please subscribe to our ${channelWord}. ` +
    `Once joined, click the button below.\n\n` +
    `<b>Action:</b> Muted \u{1F507} until <b>${untilText}</b>.`;

  const buttons: any[] = [];
  if (config.forceJoinInviteLink) {
    buttons.push([Markup.button.url("\u{1F4E2} Subscribe to channel 1", config.forceJoinInviteLink)]);
  }
  if (config.forceJoinInviteLink2) {
    buttons.push([Markup.button.url("\u{1F4E2} Subscribe to channel 2", config.forceJoinInviteLink2)]);
  }
...
export function registerForceJoin(bot: Telegraf): void {
  ...
  bot.on("new_chat_members", async (ctx) => {
    ...
    for (const member of newMembers) {
      ...
      const alreadyJoined = await hasJoinedAllRequired(ctx.telegram, config, member.id);
  ...
  bot.on("message", async (ctx, next) => {
    const config = getChatConfig(ctx.chat.id);
    if (!config.forceJoinChannel && !config.forceJoinChannel2) return next();
    ...
    const joined = await hasJoinedAllRequired(ctx.telegram, config, userId);
  ...
  bot.action(/^verify_join:(\d+)$/, async (ctx) => {
    ...
    const config = getChatConfig(chatId);
    if (!config.forceJoinChannel && !config.forceJoinChannel2) {
      return ctx.answerCbQuery("Force Join is no longer active here.");
    }

    const joined = await hasJoinedAllRequired(ctx.telegram, config, targetId);
    if (!joined) {
      const channelWord = config.forceJoinChannel && config.forceJoinChannel2 ? "channels" : "channel";
      return ctx.answerCbQuery(`You haven't joined both required ${channelWord} yet. Join, then tap this again.`, {
        show_alert: true,
      });
    }
