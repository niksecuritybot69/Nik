...
export function registerVerify(bot: Telegraf): void {
  ...
  bot.command("unverify", async (ctx) => {
    if (!ctx.from || !isVerifyAdmin(ctx.from.id)) {
      return ctx.reply("🚫 You're not authorized to use /unverify.");
    }

    const usage =
      "Usage:\n" +
      "/unverify upi <address>\n" +
      "/unverify crypto <address>\n" +
      "Example: /unverify crypto 0xabc123...";
    ...
    const config = getChatConfig(chatId);
    const key = normalizeAddress(address);
    const record = config.verifiedAddresses[key];

    if (!record) {
      const label = kind === "upi" ? "UPI" : "crypto address";
      return ctx.reply(`⚠️ No verified ${label} found matching: ${address}`);
    }

    delete config.verifiedAddresses[key];
    saveChatConfig(chatId, config);

    const removedBy = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name;

    await ctx.replyWithHTML(
      `🗑 <b>ADDRESS UNVERIFIED</b>\n\n` +
        `🛡 Address or upi : <code>${escapeHtml(address)}</code>\n` +
        `🛡 Type: ${escapeHtml(record.type)}\n` +
        `👤 Was Belonging To: ${escapeHtml(record.belongsTo)}\n\n` +
        `⚠️ This address is no longer officially registered.`
    );

    await logToChannel(
      bot,
      `🗑 UNVERIFY: ${removedBy} (${ctx.from.id}) removed a ${record.type} address in chat ${chatId} ` +
        `that belonged to ${record.belongsTo}: ${address}`
    );
  });
